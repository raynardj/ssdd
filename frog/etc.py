...
def summarise_roc_label(auc, result):
    ...
    for c,(a, eer, threshold_eer) in enumerate(auc):
        text += 'class%d  : auc = %0.5f,  eer = %0.5f (%3d)'%((c+1), a, eer, threshold_eer)
        text +='\n'
    text += '\n'
...
def compute_metric_mask(truth, predict):
    ...
    for c in range(4):
        ...
        result.append([
            num_neg,  num_neg/num,  num_pos, num_pos/num,
            (dice_neg+dice_pos)/num,dice_neg/num_neg,dice_pos/num_pos,
            (hit_neg+hit_pos)/num,hit_neg/num_neg,hit_pos/num_pos,
            hit_neg/predict_neg, hit_pos/predict_pos
        ])
...
def summarise_metric_mask(kaggle, result):
    ...
    text += '--------------------------------------------------------------------------------------------------------\n'
    text += 'class1  %4d(%0.2f)  %4d(%0.2f)  |  %0.2f   %0.2f   %0.2f   |  %0.2f   %0.2f   %0.2f   |  %0.3f   %0.3f\n'%(*result[0],)
    text += 'class2  %4d(%0.2f)  %4d(%0.2f)  |  %0.2f   %0.2f   %0.2f   |  %0.2f   %0.2f   %0.2f   |  %0.3f   %0.3f\n'%(*result[1],)
    text += 'class3  %4d(%0.2f)  %4d(%0.2f)  |  %0.2f   %0.2f   %0.2f   |  %0.2f   %0.2f   %0.2f   |  %0.3f   %0.3f\n'%(*result[2],)
    text += 'class4  %4d(%0.2f)  %4d(%0.2f)  |  %0.2f   %0.2f   %0.2f   |  %0.2f   %0.2f   %0.2f   |  %0.3f   %0.3f\n'%(*result[3],)
...
def draw_predict_result_label(image, truth_label, truth_mask, probability_label):
    ...
    for c in range(4):
        ...
        if (probability_label[c]>0.5)*(truth_label[c]<0.5): asterisk='FP!'
        if (probability_label[c]<0.5)*(truth_label[c]>0.5): asterisk='MISS!'
        draw_shadow_text(overlay,'pos%d %d %0.2f %s  '%(c+1,truth_label[c],probability_label[c],asterisk),(5,(c+1)*30), 1,color[c+1],2)
...
def draw_predict_result_label_one_defect_only(image, truth_label, truth_mask, probability_label):
    color = DEFECT_COLOR
    H,W,C   = image.shape
    truth_label = truth_label.reshape(-1)
    truth_mask  = truth_mask.reshape(H,W)
    ...
    if 1:
        ...
        if (probability_label>0.5)*(truth_label<0.5): asterisk='FP!'
        if (probability_label<0.5)*(truth_label>0.5): asterisk='MISS!'
        draw_shadow_text(overlay,'pos%d %d %0.2f %s  '%(1,truth_label,probability_label,asterisk),(5,(1)*30), 1,color[1],2)
...
def draw_predict_result_attention(
    ...
    for c in range(4):
        ...
        draw_shadow_text(r,'%0.03f'%(probability_label[c]),(60,60),1,
                         int_tuple([max(probability_label[c],0.25)*255]*3),2)
...
def draw_predict_result_8cls (image, truth_label, truth_mask, truth_attention, probability_attention):
    color = DEFECT_COLOR
    H,W,C   = image.shape
    truth_label = truth_label.reshape(-1)
    truth_mask  = truth_mask.reshape(H,W)
    ...
    for c in [1,2,3,4]:
        for i in range(length):
            ...
            if (p>0.5)*(t<0.5): asterisk='FP!'
            if (p<0.5)*(t>0.5): asterisk='MISS!'
            ...
            if (p>0.5) or (t>0.5):
                clr = color[c]
            if i==0:
                draw_shadow_text(overlay1,'pos%d [%d] %0.2f %s  '%(c,t,p,asterisk),(5+i*w,(c)*30), 1,clr,2)
            else:
                draw_shadow_text(overlay1,'[%d] %0.2f %s  '%(t,p,asterisk),(5+i*w,(c)*30), 1,clr,2)
...
def draw_predict_result_softmax_label(image, truth_label, truth_mask, probability_label):
    color = DEFECT_COLOR
    H,W,C   = image.shape
    truth_label = truth_label.reshape(-1)
    truth_mask  = truth_mask.reshape(H,W)
    ...
    for c in [0]:
        ...
        draw_shadow_text(overlay,'neg  %d %0.2f  '%(is_empty,probability_label[0]),(5,30), 1,
                         color[c], 2, color1=[255,255,255], thickness1=3)

    for c in [1,2,3,4]:
        asterisk = '*' if probability_label[c]>0.5 else ''
        if (probability_label[c]>0.5)*(truth_label[c-1]<0.5): asterisk='FP!'
        if (probability_label[c]<0.5)*(truth_label[c-1]>0.5): asterisk='MISS!'
        draw_shadow_text(overlay,'pos%d %d %0.2f %s  '%(c,truth_label[c-1],probability_label[c],asterisk),(5,(c+1)*30), 1,color[c],2)
...
def draw_predict_result(image, truth_label, truth_mask, probability_label, probability_mask):
    color = DEFECT_COLOR
    H,W,C   = image.shape
    truth_label = truth_label.reshape(-1)
    truth_mask  = truth_mask.reshape(H,W)
...
def run_show_csv():
    df = pd.read_csv('/root/share/project/kaggle/2019/steel/data/semi/dump/merge-x11-14-fix-0.91262.csv').fillna('')
