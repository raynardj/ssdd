...
def run_submit_segmentation(
    ...
    if 0: #save
        ...
        if 1: #save
            ...
            np.savez_compressed(out_dir + '/submit/%s/truth_label.uint8.npz'%(mode), truth_label)
            np.savez_compressed(out_dir + '/submit/%s/truth_mask.uint8.npz'%(mode), truth_mask)
            np.savez_compressed(out_dir + '/submit/%s/probability_label.uint8.npz'%(mode), probability_label)
    ...
    if 1:
        ...
        truth_label       = np.load(out_dir + '/submit/%s/truth_label.uint8.npz'%(mode))['arr_0']
        truth_mask        = np.load(out_dir + '/submit/%s/truth_mask.uint8.npz'%(mode))['arr_0']
        probability_label = np.load(out_dir + '/submit/%s/probability_label.uint8.npz'%(mode))['arr_0']
